...
class HtmlController extends BaseController{
    ...
    private function mhomgepagehtml(){
        $this->getPrecent();
        $path = public_path('customers/'.$this->customer.'/mobile/index.html');
        $template = new PrintController('online','mobile');
        $template->mhomepagePreview();
        file_put_contents($path, ob_get_contents());
        ob_clean();
        return $path;
    }
    ...
    private function categoryhtml($ids=[]){
        $result = [];
        $template = new PrintController('online','pc');
        ...
        foreach($ids as $id){
            ...
            $this->getPrecent();
            $path = public_path('customers/'.$this->customer.'/category/'.$id.'.html');
            ...
            for($i=2;$i<=$page_count;$i++){
                $this->getPrecent();
                $path = public_path('customers/'.$this->customer.'/category/'.$id.'_'.$i.'.html');
     ...
     * @return string
     */
    private function articlehtml($ids=[]){
        $template = new PrintController('online','pc');
        $result = [];
        foreach($ids as $id){
            $this->getPrecent();
            $path = public_path('customers/'.$this->customer.'/detail/'.$id.'.html');
