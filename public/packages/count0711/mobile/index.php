...
//计数
if($now>$date){
	$sql = "update count set count_all = count_all + 1,count_mobile = count_mobile + 1,count_today = 1,record_date = '".$now."'";
}else{
	$sql = "update count set count_all = count_all + 1,count_mobile = count_mobile + 1,count_today = count_today + 1,record_date = '".$now."'";
}
$res = $db->exec($sql);
