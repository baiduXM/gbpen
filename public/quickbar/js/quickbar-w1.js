...
//增加no-cache的meta标签，取消缓存
function addnocache() {
	var meta_1 = document.createElement("meta");
	meta_1.setAttribute("http-equiv", "pragma");
	meta_1.setAttribute("content", "no-cache");

	var meta_2 = document.createElement("meta");
	meta_2.setAttribute("http-equiv", "cache-control");
	meta_2.setAttribute("content", "no-cache");

	var meta_3 = document.createElement("meta");
	meta_3.setAttribute("http-equiv", "expires");
	meta_3.setAttribute("content", "0");

	var headobj = document.getElementsByTagName("head")[0];
	headobj.appendChild(meta_1);
	headobj.appendChild(meta_2);
	headobj.appendChild(meta_3);
}
addnocache();
	...
	(function($){
		...
		$(function(){
			...
			var share_style = '<style>'
				...
				+'.quickbar-sharebox .share-cance{ height:35px; line-height:35px; overflow:hidden; background:rgba(0,0,0,0.5); text-align:center; margin-top:10px;}'
            ...
            li_btn += '<li><a href="javascript:history.go(-1)"><p class="fix_icon"><i class="iconfont">&#xe641;</i></p><p class="title">返回</p></a></li>';
			// 获取侧边导航数据
